...
use probe_rs::{
    CoreInterface, CoreStatus, Error, HaltReason, InstructionSet,
    architecture::{
...
impl SteppingMode {
    ...
    pub fn step(
        ...
        let mut return_address = read_return_address(core, program_counter)?;
                ...
                Err(error) => {
                    match error {
                        DebugError::WarnAndContinue { message } => {
                            ...
                            program_counter = core.step()?.pc;
                            return_address = read_return_address(core, program_counter)?;
                            continue;
                        }
                        other_error => {
                            core_status = core.status()?;
...
fn read_return_address(
    ...
    let return_address: u64 = core.read_core_reg(core.return_address().id())?.try_into()?;
    Ok(match core.instruction_set() {
